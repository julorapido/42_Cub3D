...
/*   Updated: 2025/01/20 17:33:02 by jsaintho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    strafe(t_cub3d *f, float rot)
{
	if(position_to_map_tiles(
		(f->player)->x + 13 *(cos(degreesToRadians((f->player->rot + FOV/2 + (rot))))),
		(f->player)->y - 13 *(sin(degreesToRadians((f->player->rot + FOV/2 + (rot))))),
		f) != 1)
	{
		(f->player)->x += roundf(6 * (

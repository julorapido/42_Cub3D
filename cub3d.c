...
static void fixedUpdate(t_cub3d *f, clock_t currentTime)
{
    for(int l = 0; l < f->entities_n; l ++)
    {
        ...
        {
            f->game_entities[l].x += (10) * (cos(degreesToRadians((f->game_entities[l].dir))));
            f->game_entities[l].y -= (10) * (sin(degreesToRadians((f->game_entities[l].dir))));
...
int	render_next_frame(t_cub3d *f)
{
    ...
    // fixedupdate
    if(f->elapsedFixedTime >= 0.02)
    {
        fixedUpdate(f, currentTime);
    }
    if (f->elapsedTime >= 0.1)
    {
